...
class PlayerEngine {
    ...
    companion object {
        ...
        public const val IS_SPECIAL_EDITION = true 
...
//  THE SMART PROXY ROUTER (Dynamic ON/OFF Switch)
        private val smartProxySelector = object : java.net.ProxySelector() {
            override fun select(uri: java.net.URI?): List<java.net.Proxy> {
                // Agar Special Edition aur Proxy ON hai, toh NekoBox/Local Xray ko bhejo
                if (IS_SPECIAL_EDITION && isProxyEnabled) {
                    return listOf(java.net.Proxy(java.net.Proxy.Type.HTTP, java.net.InetSocketAddress("127.0.0.1", 2080)))
                }
                // Warna Direct Internet use karo
            ...
            override fun connectFailed(uri: java.net.URI?, sa: java.net.SocketAddress?, ioe: java.io.IOException?) {
                // Agar local proxy crash ho jaye toh error print karega
                ioe?.printStackTrace()
            }
        }
      ...
      //  .protocols(listOf(okhttp3.Protocol.HTTP_2))
            .proxySelector(smartProxySelector)
            .dns(vpsDns)                         
