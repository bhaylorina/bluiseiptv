...
# 1. FIX MANIFEST (TS Files & Logos ke liye HTTP Allow)
manifest_code = r"""<?xml version="1.0" encoding="utf-8"?>
...
# 2. FIX MAIN ACTIVITY (Favorites Folder + Dynamic Linking)
activity_code = r"""package com.bluise.iptv
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            playlistAdapter = object : ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, displayPlaylists) {
                override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
                    ...
                    if (item == "★ Favorites") {
                         ...
                         view.textSize = 20f
                         view.setCompoundDrawablesWithIntrinsicBounds(android.R.drawable.star_big_on, 0, 0, 0)
                    } else {
            ...
            playlistListView?.setOnItemLongClickListener { _, _, position, _ ->
                if (displayPlaylists[position] != "★ Favorites") {
    ...
    fun toggleFavorite(channel: Channel) {
        ...
        // Only refresh list if we are NOT in the Favorites folder (to avoid item disappearing instantly)
        if (!isShowingFavorites) {
             filterChannels(searchBar?.text.toString())
        } else {
             channelAdapter?.notifyDataSetChanged()
        }
    }

    fun loadFavoritesFolder() {
        isShowingFavorites = true
        Toast.makeText(this, "Updating Links...", Toast.LENGTH_SHORT).show()
        
        thread {
            val favChannels = ArrayList<Channel>()
            
            // Scan ALL playlists to find the LATEST link for saved favorites
            for (playlist in playlists) {
                try {
                    val url = playlist.split("|").last()
                    ...
                    for (line in lines) {
                        ...
                        if (trim.startsWith("#EXTINF")) { 
                            ...
                            // DYNAMIC LINK LOGIC:
                            // We only check if the NAME matches our saved list.
                            // If yes, we use THIS fresh URL.
                            if (favoriteNames.contains(currentName)) {
                                favChannels.add(Channel(currentName, trim, currentLogo, currentUserAgent, currentCookie, currentLicense, isFavorite=true))
                            }
                            currentLogo = null
                            currentName = "Unknown"
                        }
            ...
            runOnUiThread {
                allChannels.clear(); allChannels.addAll(favChannels)
                channels.clear(); channels.addAll(favChannels)
                channelAdapter = ChannelAdapter(this@MainActivity, channels)
                channelListView?.adapter = channelAdapter
                flipper?.displayedChild = 1 
                if (favChannels.isEmpty()) Toast.makeText(this, "No Favorites Found!", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    fun filterChannels(query: String) {
        ...
        // Sort Favorites to top usually, but in Favorites folder they are all favorites
    ...
    fun playChannel(channel: Channel) {
        try {
            ...
            // TS FILE FIX: Allow normal playback for TS
    ...
    fun savePlaylists() { 
        getSharedPreferences("iptv", Context.MODE_PRIVATE).edit().putStringSet("list", playlists.toMutableSet()).apply() 
        updateDisplayList()
    }
    fun loadPlaylists() { 
        val s = getSharedPreferences("iptv", Context.MODE_PRIVATE).getStringSet("list", null)
        playlists.clear()
    ...
    fun updateDisplayList() {
        ...
        // Add Favorites Folder at TOP
        displayPlaylists.add("★ Favorites") 
...
print("✅ FIX APPLIED: Favorites Folder Logic & TS Support Enabled!")
