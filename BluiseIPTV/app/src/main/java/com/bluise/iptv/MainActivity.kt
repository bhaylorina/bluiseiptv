...
class MainActivity : AppCompatActivity() {
    ...
    var updateSeekRunnable: Runnable? = null
    var tvCurrentTime: TextView? = null
    var tvTotalDuration: TextView? = null
    ...
    val hideControlsRunnable = Runnable { 
        ...
        updateSeekRunnable?.let { handler.removeCallbacks(it) } // 🚀 CPU Saver
    }
    ...
    private fun setupGestures() {
        //  Volume ko smooth karne wali 'Gullak'
        var accumulatedVolumeChange = 0f

        gestureDetector = GestureDetector(this, object : GestureDetector.SimpleOnGestureListener() {
            
            // Jab naya swipe shuru ho, toh gullak zero kar do
            override fun onDown(e: MotionEvent): Boolean {
                accumulatedVolumeChange = 0f
                return true
            }
            ...
            override fun onScroll(e1: MotionEvent?, e2: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
                ...
                //  PURANA CODE: Edge Navigation Safe Margin
                val safeMargin = 100f
                if (e1.x < safeMargin || e1.x > (screenWidth - safeMargin)) {
                    return false 
                }

                if (e1.x < screenWidth / 2) {
                    //  PURANA CODE: Brightness (Left Side) - No Changes Here
                    ...
                    //  NAYA CODE: Smooth Volume logic (Right Side)
                    val volSensitivity = 50f 
                    
                    accumulatedVolumeChange += distanceY

                    if (Math.abs(accumulatedVolumeChange) > volSensitivity) {
                         val maxVol = audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC)
                         val currentVol = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC)
                         
                         val change = if (accumulatedVolumeChange > 0) 1 else -1
                         ...
                         // Volume update hone ke baad wapas zero
                         accumulatedVolumeChange = 0f
                    }
                }
                return true
            ...
            //  PURANA CODE: Single Tap UI Toggle - No Changes Here
        ...
        //  PURANA CODE: Listener attach karna
    ...
    fun setupUI() {
        ...
        // 👇 YAHAN SE NAYA SEEKBAR CODE PASTE KAREIN 👇
        
        customSeekbar = findViewById(R.id.customSeekbar)
        tvCurrentTime = findViewById(R.id.tvCurrentTime)
        tvTotalDuration = findViewById(R.id.tvTotalDuration)

        // SeekBar Controller (TV Remote & Touch dono ke liye)
        customSeekbar?.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            ...
            override fun onStartTrackingTouch(seekBar: SeekBar?) { 
                handler.removeCallbacks(hideControlsRunnable) // UI ko hide hone se roko
            }
            override fun onStopTrackingTouch(seekBar: SeekBar?) { 
                showControls() // Seek khatam hote hi wapas UI timer chalu
            }
        })

        // SeekBar ko har second aage badhane wala Engine
        updateSeekRunnable = object : Runnable {
            override fun run() {
                if (player != null && controlsContainer?.visibility == View.VISIBLE) {
                    val duration = player!!.duration
                    val position = player!!.currentPosition
                    
                    // Live TV par duration 'TIME_UNSET' hoti hai.
                    if (duration > 0 && duration != C.TIME_UNSET) {
                        customSeekbar?.max = duration.toInt()
                        customSeekbar?.progress = position.toInt()
                        ...
                        // Agar LIVE TV channel hai, toh seekbar ko disabled/zero rakho
                        customSeekbar?.progress = 0
                        customSeekbar?.isEnabled = false 
                        
                        // Live TV par time clear kar dein
                        tvTotalDuration?.text = "00:00"
                        tvCurrentTime?.text = "00:00"
                    }
                }
                handler.postDelayed(this, 1000) // Har 1 sec me update
            }
        }
        
        // 👆 YAHAN TAK NAYA SEEKBAR CODE HAI 👆
    ...
    fun showControls() {
        ...
        // 🚀 START SEEKBAR ENGINE
        updateSeekRunnable?.let { handler.post(it) }
    ...
    fun playChannel(channel: Channel) {
        ...
        lastErrorTime = 0L 
        autoRetryCount = 0 // 🚀 THE FIX: Naye channel ke liye retry count zero kar diya
            ...
            if (player == null) {
                ...
                player?.addListener(object : Player.Listener {
                    override fun onIsPlayingChanged(isPlaying: Boolean) { 
                        ...
                        // 🚀 THE FIX (Point 1): Smart Screen Lock
                        // Ab screen sirf tab ON rahegi jab video actually chal rahi ho! Paused par screen off ho sakti hai.
                        playerView?.keepScreenOn = isPlaying
                    }

                    //  BUG FIX: Grey box hide karo aur sirf 1 baar actual error Toast dikhao
                    // 🚀 BUG FIX & AUTO-RECOVERY: Grey box hide karo aur Universal Retry maaro!
                    override fun onPlayerError(error: androidx.media3.common.PlaybackException) {
                        
                        // 1. ExoPlayer ke default error UI ko chupchaap hide kar do
                        playerView?.post {
                        ...
                        if (autoRetryCount < MAX_RETRIES) {
                            ...
                            // 3. Agar 3 baar try karne par bhi server zinda nahi hua, tab asli error dikhao
    ...
    //  YAHAN AAPNE NAYA FUNCTION PASTE KIYA 
